...
export function DetailView({ instance, onBack, onExport, onUpdateInstance }: DetailViewProps) {
  ...
  return (
    <div className="animate-slide-in h-full flex flex-col" style={{ background: 'var(--bg-canvas)' }}>
      <DetailHeader 
        instance={instance} 
        onBack={onBack} 
      ...
      <div className="px-6 mt-6 flex gap-6 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        {[{ key: 'overview', label: 'Overview' }, { key: 'client', label: 'Client Mods' }, { key: 'server', label: 'Server Files' }].map(tab => (
          <button 
            key={tab.key} 
